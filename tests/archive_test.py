...
class ArchiveTest(ProbackupTest):
    ...
    def test_undefined_wal_file_path(self):
        ...
        archive_command = " ".join([f'"{self.probackup_path}"', 'archive-push',
                                    *backup_dir.pb_args, '--instance=node',
                                    '--wal-file-name=%f'])
        if os.name == 'nt':
            archive_command = archive_command.replace("\\","\\\\")

        node.set_auto_conf({'archive_command': archive_command})
        ...
        # check
        self.assertEqual(self.pb.show_archive(instance='node', tli=1)['min-segno'],
                         '000000010000000000000001')
...
#                '--recovery-target-action=promote',
    ...
    def test_archive_get_relative_path(self):
        ...
        relative_pgdata = str(PurePath(restored.data_dir).relative_to(os.getcwd()))
