...
#define STEPBACK_CHUNK (1024*1024)
/*
...
{
	XLogRecPtr	startpoint = stop_lsn - (stop_lsn % STEPBACK_CHUNK);
	...
	do
	{
		...
		do {
			...
			if (record == NULL)
			{
				...
				errptr = curpoint ? curpoint : xlogreader->EndRecPtr;
		...
		startpoint -= STEPBACK_CHUNK;
	} while (startpoint + STEPBACK_CHUNK >= start_lsn);
