...
static XLogRecPtr pg_start_backup(const char *label, bool smooth, pgBackup *backup,
...
do_backup_instance(PGconn *backup_conn, PGNodeInfo *nodeInfo, bool no_sync, bool backup_logs)
{
	...
	current.start_lsn = pg_start_backup(label, smooth_checkpoint,
										nodeInfo, backup_conn);
...
/* */
static void
do_catchup_instance(PGconn *backup_conn, PGNodeInfo *nodeInfo,
					CatchupParams *catchupParams)
{
	int			i;
	char		database_path[MAXPGPATH];
	...
	if (stream_wal)
	{
		...
		start_WAL_streaming(backup_conn, dst_catchup_wal_path, &instance_config.conn_opt,
							current.start_lsn, current.tli);
	}
 ...
 */
static XLogRecPtr
pg_start_backup(const char *label, bool smooth,
				PGNodeInfo *nodeInfo, PGconn *conn)
{
	PGresult   *res;
	const char *params[2];
