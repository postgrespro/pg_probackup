...
open_walfile(StreamCtl *stream, XLogRecPtr startpoint)
{
		...
		stream->walmethod->existsfile(fn))
	{
		...
		if (size < 0)
		{
			elog(ERROR, "could not get size of write-ahead log file \"%s\": %s",
		...
		if (size != 0)
		{
			...
			if (errno == 0)
				errno = ENOSPC;
			elog(ERROR, ngettext("write-ahead log file \"%s\" has %d byte, should be 0 or %d",
...
close_walfile(StreamCtl *stream, XLogRecPtr pos)
{
	...
	if (currpos == -1)
	{
		elog(ERROR, "could not determine seek position in file \"%s\": %s",
